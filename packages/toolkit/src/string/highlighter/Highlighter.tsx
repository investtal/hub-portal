...
import memoizeOne from "./memoize-core"
...
interface HighlighterProps {
  ...
  className?: string
  findChunks?: (options: any) => Array<{ start: number; end: number }>
  highlightClassName?: string | object
  highlightStyle?: object
  highlightTag?: React.ComponentType<any> | string
  sanitize?: (text: string) => string
  searchWords: Array<string>
  textToHighlight: string
  unhighlightTag?: React.ComponentType<any> | string
    ...
    chunks.map((chunk, index) => {
      ...
      if (chunk.highlight) {
        ...
        if (typeof highlightClassName === "object") {
          const classNameMap = !caseSensitive
            ? memoizedLowercaseProps(highlightClassName)
            : highlightClassName
          highlightClass = classNameMap[!caseSensitive ? text.toLowerCase() : text]
        } else {
        ...
        return createElement(HighlightTag, props, text)
      }
